...
- Fixed gear disappearing when a player walked out of range after being scanned. An inspect on someone who has drifted away still reports success, but hands back no items at all, and that empty result was being written straight over the good scan - the row kept saying `ready` while the item list, GS and audit all went blank. A scan that returns no gear no longer replaces gear that is already stored; the row shows `out of range, retrying` and keeps the equipment on screen.
- Once a player's gear has been collected they are never re-inspected on their own again. Autoscan still watches the roster and picks up anyone who joins, but it no longer re-scans people it already has gear for, and the retry sweep leaves them alone too. Re-scanning someone is now always a deliberate act: the row `Refresh` button, the `Raid` button, or `/ri refreshstale`. This removes the loop where a scanned player standing right next to you would flip to `out of range, retrying`, re-scan, and repeat every few seconds.
- Anyone with no GS is still re-inspected automatically until a scan actually lands, including rows left blank by the bug above, so they fill in by themselves once the player comes into range.
